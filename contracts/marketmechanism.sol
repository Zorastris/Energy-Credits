...
EnergyCredits energytoken = EnergyCredits(ckaddress);
...
function sort_arrayauf() private{
    ...
    for(uint i = 0; i < l; i++) {
        for(uint j = i+1; j < l ;j++) {
            if(getBidPricepv(bid_ids[i]) > getBidPricepv(bid_ids[j])) {
...
function matchingTransactions() private {
    ...
    for(uint z = 0; z < match_ids.length; z++){
         if(remainingLockedValue[matches[match_ids[z]].askaddress]>0){
           ...
           emit Transaction(this,matches[match_ids[z]].askaddress,"Ruckzahlung Wei",remainingLockedValue[matches[match_ids[z]].askaddress],lauf);
