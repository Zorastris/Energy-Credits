...
modifier hastokenBalance(uint _amount){
      require((credits.allowance(msg.sender,address(this)) + remainingLockedValue[msg.sender])>=_amount);
  _;
  }
...
function matchingTransactions() private {
    ...
    for(uint z = 0; z < match_ids.length; z++){
        credits.transfer(matches[match_ids[z]].askaddress,(matches[match_ids[z]].amount)); // Asker bekommt token vom contract (die wir vom Bidder bekommen haben)
...
function test() public returns (uint){
    uint value = credits.balanceOf(msg.sender);
    emit iTest(value);
    return value;
